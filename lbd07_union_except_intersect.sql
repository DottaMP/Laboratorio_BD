...
-- union - união de duas entidades 
-- precisa ter a mesma qtd de campos, na mesma ordem e o mesmo tipo.
...
select nome, email, 0 as idade, 'vendedor' as tipo from vendedor
	union -- nessa união remove os duplicados caso haja
		select nome, email, idade, 'cliente' as tipo from cliente
			order by nome;
...
-- except - Quando se quer que apareçam os registros da tabela A que não são iguais aos da tabela B, temos a opção de ‘subtrair’ os registros utilizando EXCEPT
select nome, email from vendedor 
	where (nome, email) not in -- Traz todos da tabela A que não fazem parte da tabela B
		(select nome, email from cliente);

-- intersect - permite uma intersecção entre as consultas. Somente traz os registros que as duas consultas têm em comum.
select nome, email from vendedor 
